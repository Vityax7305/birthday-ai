// src/components/EmailInviteModal.tsx
...
export function EmailInviteModal({ isOpen, onClose, guest, onInviteSent }: EmailInviteModalProps) {
  ...
  const SERVICE_ID = "service_4qjxk5w";     
  const TEMPLATE_ID = "template_qptb41k";   
  const PUBLIC_KEY = "fEXAme9xDNL66lK25";     

  const handleSendInvite = async () => {
    if (!guest || !guest.email) {
      setStatus("error");
      setMessage("У гостя нет email адреса");
      return;
    }
    ...
    try {
      const templateParams = {
        ...
        event_date: new Date().toLocaleDateString("ru-RU"),
        event_location: "По согласованию",
        invite_link: `${window.location.origin}/confirm/${guest.id}`,
        from_name: "BirthdayAI",
      };

      await emailjs.send(SERVICE_ID, TEMPLATE_ID, templateParams, PUBLIC_KEY);
      
      setStatus("success");
      setMessage(`Приглашение отправлено ${guest.name} на ${guest.email}`);
      ...
      setStatus("error");
      setMessage("Ошибка отправки. Проверьте настройки EmailJS.");
      ...
      >
        ...
        >
          ...
          <div className="text-center mb-6">
            ...
            {!guest.email && (
              <p className="text-yellow-400 text-sm mt-1">⚠️ У гостя нет email</p>
            )}
          </div>
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full py-3 rounded-xl flex items-center justify-center gap-2 text-white font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              style={{ background: "linear-gradient(135deg, #f857a6, #8b5cf6)" }}
            >
              {isSending ? (
                <>
          ...
          <p className="text-center text-white/30 text-xs mt-4">
            Приглашение будет отправлено на email гостя
          </p>
        </motion.div>
      </motion.div>
    </AnimatePresence>
